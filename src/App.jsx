...
function App() {
  ...
  const [generatedPrefix, setGeneratedPrefix] = useState(''); // для какого S/N Prefix сформированы карточки
  const [downloadPackage, setDownloadPackage] = useState(null); // { blob, filename } — готово к скачиванию отдельной кнопкой
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  // Сколько карточек выбранного типа помещается на один лист А4
  const maxCardsPerSheet = getMaxCardsPerSheet(formData.prefix).max;
  ...
  // Полный сброс: форма, превью карточек и подготовленный архив — как после перезагрузки страницы
  const handleClear = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const maxAllowed = getMaxCardsPerSheet(prefix).max;
      if (count > maxAllowed) {
        throw new Error(
          `Too many codes for one A4 sheet of this item (max ${maxAllowed}). Reduce the quantity.`
        );
      }

      const material = MATERIAL_BY_PREFIX[prefix]; // не используется для TCR/TCN/TSR/TSN
      const zip = new JSZip();
      const filesData = [];
      ...
      if (count === 1) {
        ...
        setDownloadPackage({ blob: zipBlob, filename: zipName });
      }
  ...
  // Скачивание архива (или одиночного PNG) — только по отдельной кнопке
  const handleDownloadArchive = () => {
    if (!downloadPackage) return;
    const link = document.createElement('a');
    link.href = URL.createObjectURL(downloadPackage.blob);
    link.download = downloadPackage.filename;
    link.click();
  };
        ...
        <Form>
              ...
              <Form.Group>
                ...
                <div className="d-flex align-items-center gap-2">
                  <Form.Control
                    ...
                    readOnly={!pnEditable}
                    className={!pnEditable ? 'pn-locked' : ''}
                  />
                  <Button
                    variant="outline-light"
                    size="sm"
                    className="flex-shrink-0"
                    onClick={() => setPnEditable(prev => !prev)}
                    title={pnEditable ? 'Заблокировать P/N' : 'Редактировать P/N вручную'}
                  >
            ...
            <Col md={6}>
              <Form.Group>
                ...
                <Form.Control 
                  ...
                  placeholder="20"
                  min="1"
                  max={maxCardsPerSheet}
                />
                <Form.Text className="text-muted">
                  Max {maxCardsPerSheet} pcs per A4 sheet for this item
                </Form.Text>
              </Form.Group>
            </Col>
          </Row>
          ...
          <div className="d-flex justify-content-end gap-3 mt-4">
            ...
            <Button
              ...
              disabled={!downloadPackage || loading}
              title={downloadPackage ? 'Download the generated PNG/ZIP' : 'Generate the codes first'}
            >
        ...
        <div className="mt-5">
          <div className="d-flex justify-content-between align-items-center mb-2">
            ...
            <span className="badge bg-primary">
              {generatedFiles.length}{' '}{generatedFiles.length > 1 ? 'items' : 'item'}
            </span>
          </div>
          <p className="text-muted mb-4">
            To print, press <strong>Ctrl+P</strong> (⌘+P on Mac) — only the cards below will be printed.
          </p>
          <CardsSheet prefix={generatedPrefix} cards={generatedFiles} />
        </div>
      )}
    </Container>
  );
